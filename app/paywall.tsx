      ...
      <View style={styles.content}>
        ...
        {packages.length > 0 ? (
          packages.map((pkg) => (
            <TouchableOpacity
              key={pkg.identifier}
              style={[styles.purchaseBtn, pkg.packageType === 'LIFETIME' && styles.purchaseBtnHighlight]}
              onPress={() => handlePurchase(pkg)}
              disabled={loading}
            >
              <Text style={styles.purchaseBtnTitle}>{pkg.product.title}</Text>
              <Text style={styles.purchaseBtnPrice}>{pkg.product.priceString}</Text>
          ...
          <View style={styles.placeholderPackages}>
            ...
            <TouchableOpacity style={[styles.purchaseBtn, styles.purchaseBtnHighlight]} disabled>
              <Text style={styles.purchaseBtnTitle}>Lifetime</Text>
              <Text style={styles.purchaseBtnPrice}>IDR 149,000</Text>
              <Text style={styles.bestValue}>Best Value</Text>
            </TouchableOpacity>
          </View>
        )}
